%% Based on mod_websockets.erl

-module(mod_websockets_prebind).
-behaviour(gen_mod).
...
websocket_init(Transport, Req, Opts) ->
    ...
    {Session, NewReq1} = cowboy_req:cookie(<<"connecter_session">>, NewReq),

    NewReq2 = cowboy_req:set_resp_header("Sec-WebSocket-Protocol", "xmpp", NewReq1),
    ...
    case ejabberd_c2s:start({?MODULE, SocketData}, C2SOpts) of
        {ok, Pid} ->
            ...
            case is_binary(Session) of
                true ->
                    initiate_login(Session),
                    ok;
                _ ->
                    o
            end,
